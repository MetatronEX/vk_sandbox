...
namespace vk
{
    void swap_chain::create(uint32_t& _width, uint32_t& _height, const bool vsync)
    {
        ...
        pre_transform = surf_caps.supportedTransforms & VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR ?
        ...
        std::vector<VkCompositeAlphaFlagBitsKHR> composite_alpha_flags = {
            VK_COMPOSITE_ALPHA_BIT_KHR,
            VK_COMPOSITE_ALPHA_PRE_MULTIPLIED_BIT_KHR,
            VK_COMPOSITE_ALPHA_POST_MULTIPLED_BIT_KHR,
        ...
        uint32_t queue_familiy_index_count = 0;
        const uint32_t* queue_familiy_indices = nullptr;

        if (indices.graphics != indices.present)
        {
            sharing_mode = VK_SHARING_MODE_CONCURRENT;
            queue_familiy_index_count = 2;
            queue_familiy_indices = queue_familiy_indices.data();
        }
        ...
        CI.queueFamilyIndexCount = queue_familiy_index_count;
        CI.pQueueFamilyIndices = queue_familiy_indices;
        CI.presentMode = scpm;
        CI.oldSwapchain = old;
        CI.clipped = VK_TRUE;
    ...
    std::tuple<VkResult,uint32_t> swap_chain::acquire_next_image(VkSemaphore present_complete)
    {
        ...
        uint32_t index;
        VkResult result = vkAcquireNextImageKHR(device, swap_chain, time_out, present_complete, VK_NULL_HANDLE, &index);

        return std::tie(result,index);
    }

    void swap_chain::queue_present(VkQueue queue, const uint32_t image_index, VkSemaphore wait)
    {
        VkPresentInfoKHR PI {};
