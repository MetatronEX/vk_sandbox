                ...
                VkImageUsageFlags usage, VkImageLayout layout,bool force_linear)
        {
            ...
            if(use_staging)
            {
                ...
                for (uint32_t i = 0; i < mip_levels; i++)
                {
                    ...
                    BCR.imageSubresource.aspect = VK_IMAGE_ASPECT_COLOR_BIT;
            ...
            else
            {
                ...
                assert(FP.linearTilingFeatures & VK_FORMAT_FETURE_SAMPLED_IMAGE_BIT);

                VkImage         mappable_image;
                VkDeviceMemory  mappable_memory;

                auto IC = info::create_image_info();
                IC.imageType = VK_IMAGE_TYPE_2D;
                IC.format = format;
                IC.extent = { width, height, 1 };
                IC.mipLevels = 1;
                ...
                auto MA = info::memory_allocate_info();
                MA.allocationSize = MR.memoryRequirements.size;
                MA.memoryTypeIndex = gpu->query_memory_type(MR.memoryRequirements.memoryTypeBit, 
                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
                ...
                VkSubserouceLayout SL{};
                void* data;

                vkGetImageSubresourceLayout(gpu->device, mappable_image, &SR, &SL);
