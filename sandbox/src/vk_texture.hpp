...
namespace vk
{
	...
	template <typename image_policy>
	struct texture_2D : private texture<image_policy>
	{
		...
		size_t query_offset(image_header_ptr header, const uint32_t level)
		{
			return get_offset(header, level);
		}
        ...
        {
            image_policy::image_header_ptr header = load_image(filename);
