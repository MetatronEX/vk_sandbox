...
namespace vk
{
    VkResult GPU::create()
    {
        ...
        if(VK_QUEUE_GRAPHICS_BIT & requested_queue)
        {
        ...
        if(VK_QUEUE_COMPUTE_BIT & requested_queue)
        {
            if(queue_indices.compute.value() != queue_indices.graphics.value())
            {
                VkDeviceQueueCreateInfo DQC{};
                DQC.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
                DQC.queueFamilyIndex = queue_indices.compute.value();
                DQC.queueCount = 1;
                DQC.pQueuePriorities = &default_priority;
                DQCs.push_back(DQC);
            }
        ...
        if(VK_QUEUE_TRANSFER_BIT & requested_queue)
        {
            if(
            ...
            )
            {
                VkDeviceQueueCreateInfo DQC{};
                DQC.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
                DQC.queueFamilyIndex = queue_indices.transfer.value();
                DQC.queueCount = 1;
                DQC.pQueuePriorities = &default_priority;
                DQCs.push_back(DQC);
            }
    ...
    VkFormat GPU::query_for_depth_format_support(const bool check_sampling_support)
    {
        VkFormat f;
        bool result = query_depth_format_support(physical_device, f, check_sampling_support);
    ...
    uint32_t GPU::query_memory_type(uint32_t type_bits, VkMemoryPropertyFlags properties, bool* found)
    {
        ...
        {
            if((type_bits & (1 << i)) &&
            (device_memory_properties.memoryProperties.memoryTypes[i].propertyFlags & properties) == properties)
            {
                if(found)
                    *found = true;
    ...
    void GPU::flush_command_buffer(VkCommandBuffer commandbuffer, VkQueue queue, VkCommandPool pool, bool free)
    {
        ...
        VkFence fence;
        OP_SUCCESS(vkCreateFence(device,&FC,allocation_callbacks,&fence));
    ...
    VkResult GPU::create_buffer(VkBufferUsageFlags usage, VkMemoryPropertyFlags property, const VkDeviceSize size, VkBuffer* buffer, VkDeviceMemory* memory, void* data)
    {
        ...
        VkBufferMemoryRequirementsInfo2 BMR{};
        BMR.sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_REQUIREMENTS_INFO_2;
        BMR.buffer = *buffer;
        ...
        MA.memoryTypeIndex = query_memory_type(MR.memoryRequirements.memoryTypeBits, property);
    ...
    VkResult GPU::create_buffer(VkBufferUsageFlags usage, VkMemoryPropertyFlags property, buffer* buffer, const VkDeviceSize size, void* data)
    {
        ...
        OP_SUCCESS(vkAllocateMemory(device, &MA, allocation_callbacks, &buffer->memory));
    ...
    void GPU::setup_default_depth_stencil()
    {
        ...
        vkGetImageMemoryRequirements2(device,&IMR,&MR);

        auto MA = info::memory_allocate_info();
        MA.allocationSize = MR.memoryRequirements.size;
        MA.memoryTypeIndex = query_memory_type(MR.memoryRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    ...
    void GPU::setup_default_renderpass()
    {
        ...
        SP.pPreserveAttachments = nullptr;
